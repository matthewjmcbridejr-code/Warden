...
describe('structured provider boundary', () => {
  it('reports disconnected providers honestly', async () => {
    ...
    await expect(provider.startRun({ prompt: 'test', project: 'Warden', workingDirectory: '/tmp', authSource: 'subscription' })).rejects.toThrow('not connected');
  });
  it('keeps Codex on the App Server path', async () => {
    const provider = new CodexAppServerAdapter();
    await expect(provider.startRun({ prompt: 'test', project: 'Warden', workingDirectory: '/tmp', authSource: 'subscription' })).rejects.toThrow('App Server');
  });
});
