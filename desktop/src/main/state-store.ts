...
const defaults: DesktopState = { version: 2, onboardingComplete: false, mode: 'simple', workspace: 'team-chat', selectedProvider: 'claude', selectedPlatformId: 'platform-claude', recentProjects: [], projects: [], browserProfiles: [personalProfile], platforms: defaultPlatforms(), removedPlatforms: [], terminals: [], windowBounds: { width: 1440, height: 920 } };
function isProvider(value: unknown): value is ProviderId { return ['claude', 'chatgpt', 'gemini', 'grok'].includes(String(value)); }
function isWorkspace(value: unknown): value is WorkspaceId { return value === 'team-chat' || value === 'chat' || value === 'build'; }
