...
import type { ContextPack, NormalizedRunEvent, ProofState, ProviderAuthReport, RunApproval, WardenRun } from '../shared/types';
...
export class RunStore {
  ...
  create(input: { provider: string; project: string; projectId?: string; cwd: string; prompt: string; model?: string; context?: ContextPack; auth?: ProviderAuthReport }): WardenRun { const now = new Date().toISOString(); const run: WardenRun = { id: `run-${randomUUID()}`, provider: input.provider, model: input.model, project: input.project, projectId: input.projectId, cwd: input.cwd, prompt: redact(input.prompt), status: 'starting', auth: input.auth, createdAt: now, updatedAt: now, context: input.context, events: [], approvals: [], evidence: { branch: input.context?.branch, gitStatusBefore: input.context?.gitStatus, changedFiles: [], tests: [] }, proof: { local: 'not_saved', brain: 'not_attempted' } }; this.save(run); return run; }
