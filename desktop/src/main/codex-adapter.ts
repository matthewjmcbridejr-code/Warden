...
export class CodexAppServerProvider implements BuildProvider {
  ...
  async startRun(input: StartRunInput): Promise<RunHandle> {
    ...
    const run = this.store.create({ provider: this.id, project: input.project, cwd: input.workingDirectory, prompt: input.prompt, model: input.model, context: input.context, auth: activeAuth }); this.publish(run);
    try {
      await this.rpc.ensureStarted();
