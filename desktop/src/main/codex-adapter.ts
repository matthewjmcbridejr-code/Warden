...
export function extractErrorMessage(err: unknown): string {
  ...
  if (typeof err === 'object') {
    ...
    if (typeof obj.error === 'object' && obj.error) return extractErrorMessage(obj.error);
    try {
...
export function mapCodexNotification(method: string, params: Record<string, unknown>, runId: string): NormalizedRunEvent | null {
  ...
  if (method === 'turn/completed') {
    ...
    const final = [...items].reverse().find((candidate) => candidate.type === 'agentMessage');
    const status = String(turn.status || 'completed');
    const errorText = extractErrorMessage(turn.error);
    return { ...base, type: status === 'completed' ? 'run.completed' : status === 'interrupted' ? 'run.cancelled' : 'run.failed', payload: { turnId: turn.id, status, error: errorText || undefined, finalMessage: final?.text || '' } };
  }
  if (method === 'error') {
    const errorText = extractErrorMessage(params.error || params.message);
    return { ...base, type: 'run.failed', payload: { error: errorText || 'Codex error' } };
  }
  return null;
}
