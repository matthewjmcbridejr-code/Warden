...
function renderProgress(run: WardenRun): void {
  ...
  if (run.error) { const row = document.createElement('div'); row.className = 'sb-step sb-step-error'; const errText = typeof run.error === 'object' && run.error !== null ? ((run.error as { message?: string; detail?: string }).message || (run.error as { detail?: string }).detail || JSON.stringify(run.error)) : String(run.error); row.textContent = `Something went wrong: ${errText}`; list.append(row); }
