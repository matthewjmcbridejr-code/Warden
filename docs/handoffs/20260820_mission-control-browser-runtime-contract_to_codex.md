...
4. **Single-Use Action-Bound Approvals**: Decisions require matching `confirmation_id`, `session_id`, and `action_id`. Replay or cross-action authorization is rejected.
...
7. **Safe Artifact & Screenshot Serving**: Screen observations are served through authenticated REST endpoints (`/api/mcharness/computer/screenshots/{filename}`) with traversal protection.
...
| `computer_confirmation_resolved` | `approval_granted` / `approval_denied` | `{"session_id": str, "confirmation_id": str, "decision": "approved"|"denied", "executed": bool}` | Resumes or halts session |
...

{
  ...
  "expected_session_id": "cs_1787260500_abc123", // optional safety check
  "expected_action_id": "act_0_click"            // optional safety check
}
```
**Response (200 OK):**
```json
